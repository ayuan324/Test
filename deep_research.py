...
# os.environ['AZURE_OPENAI_API_KEY'] = "5YzhcN3wCRnFORXYl9SPWpzb7RIPRQmew0V71y0chvR6g6j8hcFOJQQJ99BDACHYHv6XJ3w3AAAAACOGFi3L"
# os.environ['AZURE_OPENAI_ENDPOINT'] = "https://ai-14911520644664ai275106389756.services.ai.azure.com/models"
...
if not tavily_api_key:
    print("[Warning] 未找到 TAVILY_API_KEY 环境变量。Tavily 搜索将无法工作。")
    tavily_client = None
else:
...
def create_llm(model_name: str = "qwen-max", temperature: float = 0.7, max_tokens: int = 20480, **kwargs):
    ...
    if model_name == "qwen-max":
        api_key = os.getenv("DASHSCOPE_API_KEY", "sk-629fea8a62e94a9cbf4ef47827ce53e4") # 优先环境变量，否则用硬编码
        if not api_key:
             raise ValueError("Missing DASHSCOPE_API_KEY for qwen-max")
        return ChatOpenAI(
            model="qwen-max-latest", # 使用 qwen-max-latest 或具体版本
        ...
        api_key = os.getenv("DEEPSEEK_API_KEY", "sk-8d71a948abc44172ae9471247099b92b")
        if not api_key:
             raise ValueError("Missing DEEPSEEK_API_KEY for deepseek-chat")
        return ChatOpenAI(
            model="deepseek-chat",
            openai_api_key=api_key,
            openai_api_base="https://api.deepseek.com/v1",
            temperature=temperature,
            max_tokens=max_tokens, # Deepseek 支持更高 token
            streaming=False,
            **kwargs
        )
        ...
        deployment_name = os.getenv("AZURE_DEPLOYMENT_NAME", "gpt-4o") # 允许覆盖部署名
        api_version = os.getenv("AZURE_API_VERSION", "2024-02-15-preview") # 使用较新的稳定版本

        if not all([api_key, azure_endpoint]):
            raise ValueError("Missing AZURE_OPENAI_API_KEY or AZURE_OPENAI_ENDPOINT for Azure OpenAI")
        ...
        return AzureChatOpenAI(
            ...
            api_key=api_key,
            temperature=temperature,
            max_tokens=min(max_tokens, 4096), # GPT-4o (Azure) 通常有 token 限制
            streaming=False,
            **kwargs
        )
    else:
...
if __name__ == "__main__":
    ...
    if not tavily_api_key:
        print("错误：请先设置 TAVILY_API_KEY 环境变量再运行。")
    else:
        # --- 为命令行测试创建模型 ---
        try:
            # 可以修改这里的默认值进行命令行测试
            ...
            # 确保相关的 API Key 和 Endpoint 已设置 (环境变量或硬编码在 create_llm 中)
